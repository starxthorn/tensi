...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  return (
    <ShaderMaterial
      source={`precision mediump float; in vec2 fragCoord;
      ...
      switch (uniform.type) {
        ...
        case "uniform3f":
          preparedUniforms[uniformName] = {
            value:
              Array.isArray(uniform.value) && uniform.value.length === 3
                ? (uniform.value as number[]) // Keep it as number[] if valid
                : [0, 0, 0], // Default to [0, 0, 0] if invalid
            type: "3f",
          };
          break;
        case "uniform1fv":
        ...
        case "uniform2f":
          preparedUniforms[uniformName] = {
            value:
              Array.isArray(uniform.value) && uniform.value.length === 2
                ? (uniform.value as number[]) // Keep it as number[] if valid
